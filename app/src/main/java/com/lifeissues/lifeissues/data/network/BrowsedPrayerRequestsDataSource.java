...
import static com.lifeissues.lifeissues.ui.fragments.PrayerFragment.prayerFragment;
...
public class BrowsedPrayerRequestsDataSource extends PageKeyedDataSource<Integer, PrayerRequest> {
    private static final String LOG_TAG = BrowsedPrayerRequestsDataSource.class.getSimpleName();
    ...
    private static BrowsedPrayerRequestsDataSource sInstance;
    //private final Context mContext;
    //private final AppExecutors mExecutors;

    private List<PrayerRequest> prayerRequestList = new ArrayList<PrayerRequest>();
    // LiveData storing the latest downloaded jobs list
    private final MutableLiveData<List<PrayerRequest>> mRequestsForBrowsing;
    private SessionManager sessionManager;

    public BrowsedPrayerRequestsDataSource() {
        //mContext = context;
        //mExecutors = executors;
        mRequestsForBrowsing = new MutableLiveData<>();
        // Session manager
     ...
     */
    public static BrowsedPrayerRequestsDataSource getInstance() {
        Log.d(LOG_TAG, "Getting the network data source");
        if (sInstance == null) {
            synchronized (LOCK) {
                sInstance = new BrowsedPrayerRequestsDataSource();
    ...
    //returned requests for browsing
    public LiveData<List<PrayerRequest>> getPrayerRequestsForBrowsing() {
        return mRequestsForBrowsing;
    }
     ...
     */
    @Override
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, PrayerRequest> callback) {
        ...
        //defining the call
        Call<UserActions> call = service.getAllPrayerRequests(FIRST_PAGE, PAGE_SIZE);
        ...
        call.enqueue(new Callback<UserActions>() {
            ...
            public void onResponse(Call<UserActions> call, Response<UserActions> response) {
                ...
                try{
                    if (response.body() != null && response.body().getBrowsedRequestsList() != null) {
                        Log.d(LOG_TAG, "Initial JSON not null");
                        if (response.body().getBrowsedRequestsList().size() == 0) {
                            Log.d(LOG_TAG, "List is empty (0)");
                            prayerFragment.isListEmpty(true);
                        } else {
                            prayerRequestList = response.body().getBrowsedRequestsList();
                            //callback.onResult(response.body().getBrowsedJobsList(), null, PAGE_SIZE);
                            callback.onResult(prayerRequestList, null, FIRST_PAGE + 1);
                            prayerFragment.isListEmpty(false);
                        }
                        mRequestsForBrowsing.postValue(prayerRequestList);
                    }
                }catch (Exception e){
    ...
    @Override
    public void loadBefore(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, PrayerRequest> callback) {
        ...
        //defining the call
        Call<UserActions> call = service.getAllPrayerRequests(params.key, PAGE_SIZE);
        ...
        call.enqueue(new Callback<UserActions>() {
            ...
            public void onResponse(Call<UserActions> call, Response<UserActions> response) {
                ...
                if (response.body() != null && response.body().getBrowsedRequestsList() != null) {
                    ...
                    prayerRequestList = response.body().getBrowsedRequestsList();
                    callback.onResult(prayerRequestList, adjacentKey);
                    prayerFragment.isListEmpty(false);
    ...
    @Override
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, PrayerRequest> callback) {
        //Defining retrofit com.emtech.retrofitexample.api service
        APIService service = new LocalRetrofitApi().getRetrofitService();
